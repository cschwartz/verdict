...
**System (CMDB)**
- System ID (internal system ID), hostname, primary FQDN
...
**Deliverables:**
- OpenAPI specs for mock external sources (asset inventory, CMDB, GRC/OSCAL tool)
- Mock external services via Prism (serves realistic responses from OpenAPI specs)
- Pull-based ingestion services for all three sources (asset inventory, CMDB, OSCAL)
- Database schema design (policies, standards, statements, assets, systems, users, roles, audit logs)
- Gold source ID pattern implementation (dual-path query support)
- API key authentication and RBAC middleware
- Schema mismatch detection and logging (fatal errors logged to audit log)
- Comprehensive unit and integration tests for schema validation and ingestion logic
...
**Deliverables:**
- Check model and TagMapping model (database schema, loaded from config repo on sync)
